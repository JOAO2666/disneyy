...
export const fetchDetailsFromId = async ({ signal, detailsId, resourceType }) => {
	...
	try {
		const response = await fetch(`${BASE_URL}/${resourceType}/${detailsId}?api_key=${API_KEY}&language=${tmdbLang}&append_to_response=videos,images,external_ids&include_image_language=en,null&include_video_language=en,pt,es,fr,null`, {
			signal,
		})
		...
		const filteredData = {
			...
			imdb_id: imdbId ? (imdbId.startsWith('tt') ? imdbId : `tt${imdbId}`) : null,
		}
